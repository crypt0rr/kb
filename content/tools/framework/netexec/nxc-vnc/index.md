...
```plain
nxc vnc [-h] [-id CRED_ID [CRED_ID ...]] [-u USERNAME [USERNAME ...]] [-p PASSWORD [PASSWORD ...]] [--ignore-pw-decoding] [-k] [--no-bruteforce] [--continue-on-success] [--use-kcache] [--log LOG]
               ...
               [--server {http,https}] [--server-host HOST] [--server-port PORT] [--connectback-host CHOST] [--port PORT] [--vnc-sleep VNC_SLEEP] [--screenshot] [--screentime SCREENTIME]
               target [target ...]
```
...
options:
  ...
  --port PORT           Custom VNC port
  --vnc-sleep VNC_SLEEP
                        VNC Sleep on socket connection to avoid rate limit
